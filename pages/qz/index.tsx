...
const SelectItem = styled(FlexDiv, {
    w: 108,
    h: 42,
    ff: '$fr',
    userSelect: 'none',
...
const Tabs = ({ css = {}, list, active, onClickItem }: { css?: any; list: string[]; active: number; onClickItem: (e: any) => void }) => {
    const [listActive, setListActive] = useState(0);
    const [listShow, setListShow] = useState(false);
    return (
        <TabWrap css={css}>
            {list.map((l, idx) => (
                <FlexDiv css={{ position: 'relative' }} key={l}>
                    <TabItem
                        active={idx === active}
                        onClick={() => {
                            onClickItem(idx);
                    ...
                    {idx === 2 && listShow ? (
                        <FlexDiv css={{ position: 'absolute', flexDirection: 'column', top: 45, left: 0, zIndex: 3 }}>
...
function JobItem({ item, activeIndex }: JobIProps) {
    const { job, subs, money, id, time, company, companyText, fl = [], headImg, hr, onLine, isZxJl, isTdSuccess, ckTime } = item;
        ...
        <JobItemWrap>
            <FlexDiv css={{ justifyContent: 'space-between', alignItems: 'flex-start', pb: 20, borderBottom: '1px dashed rgba(0,0,0,0.1)' }}>
                ...
                <FlexDiv css={{ flexDirection: 'column', justifyContent: 'center', w: 206, alignItems: 'flex-end' }}>
                    ...
                    <TitleText css={{ color: '#616A67', fs: 16, ff: '$fr', fw: 400, mt: 16 }}>使用简历: {isZxJl ? '在线简历' : '附件简历'}</TitleText>
                </FlexDiv>
            </FlexDiv>
            <FlexDiv css={{ justifyContent: 'space-between', mt: 13, mr: activeIndex === 0 ? 210 : 0 }}>
                <FlexDiv css={{ w: 282 }}>
...
function JobInterviewItem({ item }: JobInterViewProps) {
    const { job, subs, money, id, time, company, companyText, fl = [], headImg, hr, onLine, isZxJl, isTdSuccess, ckTime, location } = item;

    return (
        <JobItemWrap>
            <FlexDiv css={{ justifyContent: 'space-between', alignItems: 'flex-start', pb: 20, borderBottom: '1px dashed rgba(0,0,0,0.1)' }}>
                <FlexDiv css={{ justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <FlexDiv css={{ w: 455, alignItems: 'center' }}>
                        <Image src={headImg} width={58} height={58} alt='xxx' />
                        <FlexDiv css={{ flexDirection: 'column', ml: 10 }}>
                            <TitleText css={{ color: '#3C4441', fs: 16, ff: '$fr', fw: 400 }}>{company}</TitleText>
                            <TitleText css={{ color: '#616A67', fs: 14, ff: '$fr', fw: 400, mt: 16 }}>{companyText}</TitleText>
                    ...
                    <FlexDiv css={{ flexDirection: 'column', w: 125, mr: 12 }}>
                        <FlexDiv css={{ alignItems: 'center' }}>
                            <TitleText css={{ fs: 18, mr: 10 }}>{job}</TitleText>
                        </FlexDiv>
                        <FlexDiv css={{ alignItems: 'center', mt: 13 }}>
                ...
                <FlexDiv css={{ flexDirection: 'column', justifyContent: 'center', w: 215, alignItems: 'flex-end' }}>
                    <FlexDiv css={{ alignItems: 'center' }}>
                        <TitleText css={{ color: isTdSuccess ? '$primary' : '#FF2000', fs: 16, ff: '$fr', fw: 400, textAlign: 'right' }}>
                            {isTdSuccess ? '已接受' : '待确认'}
                        </TitleText>
                    </FlexDiv>
            ...
            <FlexDiv css={{ mt: 13, mr: 215 }}>
                <FlexDiv css={{ w: 455, alignItems: 'center', color: '#616A67', fs: 14, ff: '$fr' }}>
                    面试地址：
    ...
    return (
        <RightWrap>
            ...
            {tabList.length > 0 ? (
                ...
                <FlexDiv css={{ fs: 18, color: '#3C4441', flexDirection: 'column', alignItems: 'center', fw: 600 }}>
